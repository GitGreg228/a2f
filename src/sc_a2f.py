...
def lambdas_a2f(a2f, dtype=np.float32):
    """
...
def wlogs_a2f(a2f, dtype=np.float16):
    ...
    for i in range(1, len(freqs)):
        ...
        #wlog_lambda[i] = np.exp(2 / lambda_lambda * np.sum(a2f_lambda[:i + 1] * np.log(freqs[:i + 1] / k_K_THz) / freqs[:i + 1]) * dw)
        wlog_lambda[i] = np.exp(2 / lambda_lambda * integrate.simps(a2f_lambda[:i + 1] * np.log(freqs[:i + 1] / k_K_THz) / freqs[:i + 1], x=freqs[:i + 1]))
        # wlog_gamma[i] = np.exp(2 / lambda_gamma * np.sum(a2f_gamma[:i + 1] * np.log(freqs[:i + 1] / k_K_THz) / freqs[:i + 1]) * dw)
...
def w2s_a2f(a2f, dtype=np.float16):
    ...
    for i in range(len(freqs)):
        ...
        w2_lambda[i] = np.sqrt(2 / lambda_lambda * integrate.simps(a2f_lambda[:i + 1] * freqs[:i + 1] / k_K_THz ** 2, x=freqs[:i + 1]))
        # w2_gamma[i] = np.sqrt(2 / lambda_gamma * np.sum(a2f_gamma[:i + 1] * freqs[:i + 1] / k_K_THz ** 2) * dw)
        w2_gamma[i] = np.sqrt(2 / lambda_gamma * integrate.simps(a2f_gamma[:i + 1] * freqs[:i + 1] / k_K_THz ** 2, x=freqs[:i + 1]))
    out_dict = {
